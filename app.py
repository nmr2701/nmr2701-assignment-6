...
def generate_plots(N, mu, sigma2, S):
    ...
    X = np.random.rand(N)  
    Y = np.random.normal(mu, np.sqrt(sigma2), N)  # Replace with code to generate Y values

    # TODO 2: Fit a linear regression model to X and Y
    # Hint: Use Scikit Learn
    model = LinearRegression()  # Initialize model
    model.fit(X.reshape(-1, 1), Y)  # Fit model to X and Y
    ...
    for _ in range(S):
        ...
        sim_model.fit(X_sim.reshape(-1, 1), Y_sim)  # Fit model to X_sim and Y_sim
